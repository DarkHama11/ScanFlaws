"""
Checks de seguridad para IAM - Fase 1: Identity
ScanFlaws - Módulo IAM (11 checks)
"""
import sys, os
...
def check_users_without_mfa():
    ...
    findings = []
    try:
        print("[*] Verificando usuarios sin MFA...")
...
def check_old_access_keys(max_age_days=90):
    ...
    findings = []
    try:
        print("[*] Verificando access keys antiguas...")
        paginator = client.get_paginator('list_users')
        for page in paginator.paginate():
            for user in page['Users']:
                username = user['UserName']
                try:
                    ...
                    for key in keys:
                        ...
                        if age > max_age_days:
                            findings.append(format_finding(
                                check_name="Access Key antigua",
                                entity=username,
                                issue=f"Access Key con {age} días (>{max_age_days})",
                                severity="MEDIUM",
                                extra_data={"key_id": key['AccessKeyId'], "age_days": age}
                            ))
                except ClientError:
                    continue
        print(f"[+] Check completado: {len(findings)} hallazgos")
...
def check_wildcard_policies():
    ...
    findings = []
    try:
        print("[*] Verificando políticas con wildcard...")
            ...
            for policy in page['Policies']:
                try:
                    ...
                    if _has_wildcard_resource(version['PolicyVersion']['Document']):
                        findings.append(format_finding(
                            check_name="Política wildcard",
                            entity=policy['PolicyName'],
                            issue="Política con Resource: '*'",
                            severity="MEDIUM",
                            extra_data={"arn": policy['Arn']}
                        ))
                except ClientError:
                    continue
        print(f"[+] Check completado: {len(findings)} hallazgos")
...
def check_users_with_direct_admin_policies():
    ...
    findings = []
    try:
        print("[*] Verificando políticas admin directas...")
        paginator = client.get_paginator('list_users')
        for page in paginator.paginate():
            for user in page['Users']:
                username = user['UserName']
                try:
                    ...
                    for p in policies:
                        if 'AdministratorAccess' in p['PolicyName']:
                            findings.append(format_finding(
                                check_name="Admin directo",
                                entity=username,
                                issue="Política administrativa asignada directamente",
                                severity="HIGH",
                                extra_data={"policy": p['PolicyName']}
                            ))
                except ClientError:
                    continue
        print(f"[+] Check completado: {len(findings)} hallazgos")
...
def check_root_without_mfa():
    ...
    findings = []
    try:
        print("[*] Verificando MFA en cuenta root...")
        ...
        for row in rows:
            if row['user'] == '<root_account>':
                if row['mfa_active'] == 'false':
                    findings.append(format_finding(
                        check_name="Root sin MFA",
                        entity="<root_account>",
                        issue="Cuenta root sin MFA habilitado",
                        severity="CRITICAL"
                    ))
        print(f"[+] Check completado: {len(findings)} hallazgos")
    except Exception as e:
...
def check_roles_with_dangerous_trust_policy():
    ...
    findings = []
    try:
        print("[*] Verificando trust policies peligrosas...")
        sts = get_sts_client()
                    ...
                    if isinstance(principal, dict):
                        ...
                        for arn in aws_principals:
                            if arn == "*":
                                ...
                                findings.append(format_finding(
                                    check_name="Trust policy externa",
                                    entity=role_name,
                                    issue="Trust policy permite asumir desde cuenta externa",
                                    severity="HIGH",
                                    extra_data={"principal": arn}
                                ))
        print(f"[+] Check completado: {len(findings)} hallazgos")
    except Exception as e:
...
def check_inactive_users(max_inactivity_days=90):
    ...
    findings = []
    try:
        print("[*] Verificando usuarios inactivos...")
        client.generate_credential_report()
        for _ in range(10):
            try:
                report = client.get_credential_report()
...
def check_unrestricted_passrole():
    ...
    findings = []
    try:
        print("[*] Verificando iam:PassRole sin restricciones...")
        paginator = client.get_paginator('list_policies')
        for page in paginator.paginate(Scope='Local'):
            for policy in page['Policies']:
                try:
                    ...
                    for stmt in doc.get('Statement', []):
                        ...
                        if 'iam:passrole' in actions or 'passrole' in actions:
                            ...
                            if '*' in resource:
                                findings.append(format_finding(
                                    check_name="PassRole sin restricciones",
                                    entity=policy['PolicyName'],
...
def check_unrestricted_assume_role():
    ...
    findings = []
    try:
        print("[*] Verificando sts:AssumeRole sin restricciones...")
        paginator = client.get_paginator('list_policies')
        for page in paginator.paginate(Scope='Local'):
            for policy in page['Policies']:
                try:
                    ...
                    for stmt in doc.get('Statement', []):
                        ...
                        if 'sts:assumerole' in actions or 'assumerole' in actions:
                            ...
                            if '*' in resource:
                                findings.append(format_finding(
                                    check_name="AssumeRole sin restricciones",
                                    entity=policy['PolicyName'],
...
def check_inline_policies():
    ...
    findings = []
    try:
        print("[*] Verificando políticas en línea...")
        # Usuarios
                ...
                try:
                    ...
                    if policies['PolicyNames']:
                        for name in policies['PolicyNames']:
                            findings.append(format_finding(
                                check_name="Política en línea",
                                entity=user['UserName'],
                                issue="Usa política en línea",
                                severity="LOW",
                                extra_data={"type": "Usuario", "policy": name}
                            ))
                except ClientError:
                    continue
                    ...
                    if policies['PolicyNames']:
                        for name in policies['PolicyNames']:
                            findings.append(format_finding(
                                ...
                                issue="Usa política en línea",
                                severity="LOW",
                                extra_data={"type": "Rol", "policy": name}
                            ))
                except ClientError:
                    continue
        print(f"[+] Check completado: {len(findings)} hallazgos")
...
def check_cloudtrail_disable_permissions():
    ...
    try:
        print("[*] Verificando permisos peligrosos en CloudTrail...")
        paginator = client.get_paginator('list_policies')
        for page in paginator.paginate(Scope='Local'):
            for policy in page['Policies']:
                try:
                    ...
                    for stmt in doc.get('Statement', []):
                        ...
                        if not isinstance(actions, list):
                            actions = [actions]
                        actions = {a.lower() for a in actions}
                        if actions & dangerous_actions:
                            findings.append(format_finding(
                                check_name="CloudTrail riesgo",
                                entity=policy['PolicyName'],
